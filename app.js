...
const viewNames = { dashboard: 'Overview', directory: 'Unit workspace', operations: 'Operations board', automations: 'Automation Center', integrations: 'Connections' };
...
function dashboard() {
  ...
  return `<section class="view"><div class="page-hero"><div><p class="eyebrow accent-eyebrow">Live operations view</p><h2>Start with the work that matters.</h2><p class="lede">Fuel Desk shows the units needing a decision, not every row in your roster.</p></div><button class="primary-button" data-view="operations">Open action board <span>→</span></button></div><div class="metric-grid">${metric('Action queue', attention.length, 'Units needing a decision', 'danger')}${metric('Fuel coverage', `${coverage}%`, `${metrics.arranged} units ready`, 'good')}${metric('Toll reviews', metrics.tollIssues, 'Still need verification', 'warning')}${metric('Compliance flags', metrics.complianceIssues, 'Drivers need follow-up', 'warning')}</div><div class="focus-grid"><section class="panel action-panel"><div class="panel-heading"><div><p class="eyebrow">Do next</p><h3>Priority actions</h3></div><button class="text-button" data-view="directory">Open workspace →</button></div><div class="priority-list">${attention.slice(0, 5).map((record) => `<button class="priority-item" data-record-id="${record.id}"><span class="priority-avatar">${initials(record.driver)}</span><span><strong>${escapeHtml(record.unit)} · ${escapeHtml(record.driver)}</strong><small>${record.fuelStatus !== 'Arranged' ? `Fuel: ${record.fuelStatus}` : `Tolls: ${record.tollStatus}`}</small></span>${pill(record.fuelStatus !== 'Arranged' ? record.fuelStatus : record.tollStatus)}</button>`).join('')}</div></section><section class="panel health-panel"><div class="panel-heading"><div><p class="eyebrow">Today</p><h3>Readiness score</h3></div><span class="score-status">Operational</span></div><div class="score-row"><div class="score-number">${coverage}</div><div><strong>Fleet readiness</strong><p>Based on fuel coverage and toll reviews.</p></div></div><div class="progress-track"><span style="width:${coverage}%"></span></div><div class="health-notes"><span><i class="dot green"></i>${metrics.arranged} fuel-ready</span><span><i class="dot amber"></i>${metrics.pendingFuel} follow-ups</span></div></section></div></section>`;
}
...
function render() {
  document.querySelector('#page-title').textContent = state.view === 'dashboard' ? 'Good morning, operator.' : viewNames[state.view];
